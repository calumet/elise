...
export const InsideToggleGroup: React.Context<boolean> = React.createContext(false);
...
function ToggleGroup({ className, ...props }: ToggleGroupProps): React.JSX.Element {
  return (
    <InsideToggleGroup.Provider value={true}>
      <ToggleGroupPrimitive.Root
        data-slot="toggle-group"
        className={cn("flex gap-1", className)}
        {...props}
      />
    </InsideToggleGroup.Provider>
  );
}
