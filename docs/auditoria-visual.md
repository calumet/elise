...
la mitad de los defectos de alineacion y de recorte solo existen con el menu
desplegado.
...
| Chequeo           | Que busca                                                                         |
| ----------------- | --------------------------------------------------------------------------------- |
| **Radios**        | Radios fuera de la escala de tokens — delatan un valor escrito a mano             |
...
Las escalas viven al principio del script y tienen que seguir a `elise.css`: si
cambia un token de radio o de tipografia, se cambia ahi tambien.

## Por que cada chequeo

Ninguno es hipotetico. Cada uno esta porque el defecto que busca se colo de
verdad y se encontro tarde:
...
- El **contraste** encontro `destructive` solido en oscuro a 4.02:1, y despues
  los `outline` de Badge y Button usando el color solido como texto — `warning`
  media 2.18:1.

## Falsos positivos ya descartados

Vale la pena conocerlos antes de agregar chequeos nuevos, porque son las
trampas en las que ya cayo esta herramienta:
...
- **Superposicion deliberada.** Una X de limpiar posicionada en absoluto sobre
  ...
  flujo normal.
- **Fondos semitransparentes.** `bg-primary/10` no es su color: hay que
  componerlo contra lo que tiene detras. Tratarlo como opaco daba 1.09:1 en
...
- **Cambiar de tema por detras del `ThemeProvider`.** Forzar `.dark` en el
  `<html>` deja la pagina a medio camino: el provider es dueño de ese atributo y
  lo reescribe. Se usa el control de la app.
